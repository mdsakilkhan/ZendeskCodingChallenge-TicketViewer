...
public class TicketViewerLite{
	...
	public static void main(String []args) {
		...
		int filesCreated = 0;
		while(true) {
			String loginInfo[] = getLoginInfo(input);
			
			if(!loginInfo[0].equals("")){
				createFile(loginInfo[3], fileName);
				filesCreated = 1;
				System.out.printf("%nRunning... %s%n", loginInfo[3]);
		...
		int currentPage = 1;
		boolean menuFlag = false; 	// false = main menu, true = full list menu		
	...
	public static String[] getLoginInfo(Scanner input) {
		...
		String loginInfo[] = {domain, email, password, ticketLink};
		return loginInfo;
	}
	 ...
	 */
	public static boolean createFile(String ticketLink, String destFileName) {		
		try {
	 ...
	 */
	public static boolean fileHasTickets(String fileName) {
		try {
	 ...
	 */
	public static long getLongFromJson(String fileName, String objectName) {
		try {
			FileReader fr = new FileReader(fileName);
			Object obj = new JSONParser().parse(fr);
			JSONObject jo = (JSONObject) obj;
	...
	public static int createAllFiles(String email, String pass) {
		try {
			...
			while (next_page != null) {
				...
				createFile(ticketLink, fileName);
	...
	/**
	 * This funtion deletes file in current directory.
	 *
	 * @param	fileName	the name of the file to be deleted
	 * @return	 			true if files deleted successfully
	 */
